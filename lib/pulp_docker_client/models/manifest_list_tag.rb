  ...
  class ManifestListTag
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `PulpDockerClient::ManifestListTag` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `PulpDockerClient::ManifestListTag`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'manifest_list')
        self.manifest_list = attributes[:'manifest_list']
      end
    end
    ...
    def list_invalid_properties
      ...
      if @manifest_list.nil?
        invalid_properties.push('invalid value for "manifest_list", manifest_list cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @manifest_list.nil?
      true
    end
    ...
    def hash
      [_href, _created, _type, _artifact, _relative_path, name, manifest_list].hash
    end
